...
QString StringHelper::ConvertToTime(const qint64 mcSeconds)
{
    CALL_IN(QString("mcSeconds=%1")
        .arg(CALL_SHOW(mcSeconds)));

    const qint64 min = mcSeconds/60;
    const QString text_min =
        (min < 10 ? "0" : "") + QString::number(min);
    const qint64 sec = mcSeconds % 60;
    const QString text_sec =
        (sec < 10 ? "0" : "") + QString::number(sec);
    const QString time = QString("%1:%2")
        .arg(text_min,
             text_sec);
...
qint64 StringHelper::ConvertToMilliSeconds(const QString & mcrTime)
{
    ...
    } else
    {
        ...
        if (match_m.hasMatch())
        {
            if (match_m.captured(1) == "-")
            {
                sign = -1;
            }
            m = match_m.captured(2).toInt();
...
qint64 StringHelper::ConvertToSeconds(const QString & mcrTime)
{
    CALL_IN(QString("mcrTime=%1")
        .arg(CALL_SHOW(mcrTime)));
    ...
    const qint64 time_s = sign * ((h * 60 + m) * 60 + s);
